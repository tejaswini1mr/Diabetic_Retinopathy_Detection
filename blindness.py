...
def LogIn():
    ...
    if u:
            ...
            if len(password):
                ...
                if g and b:
                    messagebox.showinfo('Hello ', 'Welcome to the System')
                else:
...
# root.configure(bg='#F3B63A')
C = Canvas(root, bg="blue", height=250, width=300)
filename = PhotoImage(file = "im1.png")
...
label1.grid(padx=10, pady=30, row=0, column=0, sticky='W')
...
label3.grid(padx=10, pady=10, row=2, column=0, sticky='W')
...
button3.grid(padx=20, pady=30, row=3, column=1)

button1 = Button(root, text="LogIn", command=LogIn)
button1.grid(padx=20, pady=30, row=3, column=2)

button2 = Button(root, text="Upload Image", command=OpenFile)
button2.grid(padx=20, pady=30, row=2, column=3)
